...
export async function generateStaticParams() {
  ...
  return [{ lang: "ko" }, { lang: "en" }].flatMap((l) =>
    slugs.map((slug) => ({ ...l, slug }))
  );
}
...
export async function generateMetadata({
  ...
  return {
    ...
    keywords: post.tags,
    alternates: {
      canonical: `https://chowonfnb.com/${locale}/danso/blog/${slug}`,
    },
    openGraph: {
      title: post.title,
      description: post.summary,
      url: `https://chowonfnb.com/${locale}/danso/blog/${slug}`,
      siteName: "단소상회",
      type: "article",
    ...
    <main className="min-h-dvh bg-charcoal">
      ...
      <article className="pt-14 max-w-2xl mx-auto px-6">
        ...
        <div className="py-6 flex items-center gap-2 font-sans text-[10px] tracking-[0.3em] text-cream/30">
          <Link href={`/${locale}/danso`} className="hover:text-gold transition-colors">홈</Link>
          <span>·</span>
          <Link href={`/${locale}/danso/blog`} className="hover:text-gold transition-colors">블로그</Link>
          <span>·</span>
